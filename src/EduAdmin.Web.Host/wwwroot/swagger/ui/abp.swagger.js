...
(function () {
    ...
    function loginUserInternal(tenantId, callback) {
        ...
        if (!phoneNum) {
            alert('phoneNum is required, please try with a valid value !');
            return false;
        }
        ...
        if (!identity) {
            alert('identity is required, please try with a valid value !');
            return false;
        }
        ...
        xhr.send(
            JSON.stringify(
                { phoneNum: phoneNum, password: password, identity: identity }//ÐÞ¸Ä´«²Î
            )
        );
    };

    abp.swagger.login = function (callback) {
        loginUserInternal(null, callback); // Login for tenant  
        //Get TenantId first
    ...
    abp.swagger.openAuthDialog = function (loginCallback) {
        ...
        //Inputs
        createInput(modalUxContent, 'phoneNum', 'PhoneNum');
        createInput(modalUxContent, 'identity', 'Identity');
